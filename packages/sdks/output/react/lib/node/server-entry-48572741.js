...
}, U = (t) => JSON.parse(JSON.stringify(t)), m = (t) => t != null, nt = (t) => {
...
}, gt = (t) => typeof t == "function" ? ht(t) : U(t), pt = ({
  inputs: t,
  ...e
}) => ({
  ...U(e),
...
const St = "v3", x = "builder.", bt = "options.", W = (t) => {
  ...
  return W(t);
}, A = (t) => typeof t == "number" && !isNaN(t) && t >= 0, O = (t) => {
  const {
    limit: e = 30,
    userAttributes: n,
    query: r,
  ...
  if (i.searchParams.set("apiKey", s), i.searchParams.set("limit", String(e)), i.searchParams.set("noTraverse", String(X)), i.searchParams.set("includeRefs", String(!0)), d && i.searchParams.set("locale", d), a && i.searchParams.set("enrich", String(a)), i.searchParams.set("omit", k || "meta.componentsUsed"), p && i.searchParams.set("fields", p), Number.isFinite(u) && u > -1 && i.searchParams.set("offset", String(Math.floor(u))), typeof P == "boolean" && i.searchParams.set("includeUnpublished", String(P)), h && A(h) && i.searchParams.set("cacheSeconds", String(h)), g && A(g) && i.searchParams.set("staleCacheSeconds", String(g)), w) {
    const l = v({
      sort: w
    });
    for (const S in l)
...
async function Pt(t) {
  try {
    const e = O(t), n = await Et(t);
    return It(n) ? Ot(t, n) : (f.error("Error fetching data. ", {
      url: e,
      content: n,
      options: t
    }), null);
